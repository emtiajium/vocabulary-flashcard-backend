...
export default class DefinitionRepository extends Repository<Definition> {
    ...
    async getRandomlyChosenMeanings(
        ...
        await this.query(/* sql */ `drop table if exists "${tableName}";`);
        await this.query(
            /* sql */ `
                create temporary table "${tableName}" as (
                    ...
                    from
                        "Definition"
                        inner join "Vocabulary" on "Vocabulary".id = "Definition"."vocabularyId"
                        and "Vocabulary"."cohortId" = $1
                    where
                        (
        ...
        return this.query(/* sql */ `
            select
                *
            from
                "${tableName}" tablesample bernoulli (10)
            limit
                15;
        `);
    }
